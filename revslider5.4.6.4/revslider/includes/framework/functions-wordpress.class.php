...
class RevSliderFunctionsWP {
	...
	public static function import_media( $file_url, $folder_name ) {
		...
		if ( ! file_exists( $ul_dir['basedir'] . '/' . $artDir ) ) {
			mkdir( $ul_dir['basedir'] . '/' . $artDir );
		}
		if ( ! file_exists( $ul_dir['basedir'] . '/' . $artDir . $folder_name ) ) {
			mkdir( $ul_dir['basedir'] . '/' . $artDir . $folder_name );
		}
		...
		if ( @fclose( @fopen( $file_url, "r" ) ) ) { //make sure the file actually exists
			...
			if ( $attach_data = wp_generate_attachment_metadata( $attach_id, $saveDir ) ) {
				wp_update_attachment_metadata( $attach_id, $attach_data );
			}
			if ( ! self::isMultisite() ) {
				$artDir = 'uploads/' . $artDir;
			}
	...
	public static function registerWidget( $widgetName ) {
		add_action( 'widgets_init', function ( $widgetName ) {
			if ( ! empty( $widgetName ) ) {
	...
	public static function getImageUrlFromPath( $pathImage ) {
		...
		if ( strpos( $pathLower, "http://" ) !== false || strpos( $pathLower,
				"https://" ) !== false || strpos( $pathLower, "www." ) === 0 ) {
			return ( $pathImage );
		}
	...
	public static function getCategoriesHtmlList( $catIDs, $strTax = null ) {
		...
		foreach ( $categories as $category ) {
			...
			if ( ! empty( $link ) ) {
				$thelist .= '<a href="' . esc_url( $link ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s",
						'revslider' ), $category["name"] ) ) . '" ' . $rel . '>' . $catName . '</a>';
			} else {
				$thelist .= $catName;
			}
